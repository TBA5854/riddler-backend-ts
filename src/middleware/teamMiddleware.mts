...
export async function otpValidator(req: Request, res: Response, next: NextFunction): Promise<void> {
...
export async function ownerValidator(req: Request, res: Response, next: NextFunction): Promise<void> {
    const name = req.params["tname"];
    const user: Token = jwt.verify(req.cookies["X-Auth-Token"], process.env.SECRET_KEY as string) as Token;
